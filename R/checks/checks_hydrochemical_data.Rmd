...
- PO4 measurements between IC and ICP-MS show greatest resemblance. 
...
d <- data %>%
  ...
  pivot_wider(names_from = parameter,
              values_from = value) %>%
  rowwise() %>% mutate(avg = mean(c(PO4_DA, `PO4_ICP-MS`), na.rm = T)) # PO4_IC is removed
...
# arranged plot for 3 PO4 methods
d_long <- d %>%
  pivot_longer(cols = PO4_DA:`PO4_ICP-MS`,
               names_to = "method",
               values_to = "concentration")

ggplot(d_long, aes(x = reorder(samplecode, -avg), y = concentration, color = method)) +
  geom_point() +
  geom_line(aes(x = reorder(samplecode, -avg), y = avg, group = 1), color = "black", linetype = "dashed") +
  scale_x_discrete(name = "arranged samples",
                   labels = NULL) +
...
ggplot(d, aes(x = sum_an, y = EC_calc)) +
  geom_point() +
  geom_abline(intercept = 0, linetype = "dashed") + 
  geom_smooth(method = "loess") +
  scale_x_continuous(name = "Sum anions [meq/l]") +
...
d <- data %>%
  filter(method == "DA") %>%
  mutate(detection_limit = ifelse(parameter == "NH4", 0.2, # these values are based on mg N/L!  
                                  0.027)) %>%
