...
IC <- openxlsx::read.xlsx(paste0(input, "2.Second_fieldwork/IC-Calculations_v2.1_20180316(+Ac)_Start_14-02-2023.xlsx"),
...
IC2 <- read.csv(paste0(input, "2.Second_fieldwork/AnCat Mike - 06-03-2023.csv"),
                       sep = ";") 

## third fieldwork
# IC high-low calibration run1
IC3 <- openxlsx::read.xlsx(paste0(input, "3.Third_fieldwork/Run1 - 07-02-2024/IC-Calculations_v2.1_20180316(+Ac)_Start__07-02-2024.xlsx"),
                           sheet = "Report", startRow = 2, check.names = T)
## remove remeasurements of second fieldwork! 
IC3 <- IC3[-(1:19),]

# IC high-low calibration run2
IC4 <- openxlsx::read.xlsx(paste0(input, "3.Third_fieldwork/Run2 - 16-02-2024/IC-Calculations_v2.1_20180316(+Ac)_Start__16-02-2024.xlsx"),
                           sheet = "Report", startRow = 2, check.names = T)
...
newnames <- c("samplecode", "F", "Cl", "NO2", "Br", "NO3", "PO4", "SO4", 
              "dilution_factor", "ID2", "F_round", "Cl_round", "NO2_round", 
              "Br_round", "NO3_round", "PO4_round", "SO4_round")
remove.list <- paste(c("Estim. l.o.d.", "St", "Blanco", "QC", "BGW", "BSW", "PB"), collapse = '|')

IC3 <- rbind(IC3, IC4 %>% select(-c("X18", "X19")))
...
d_IC3 <- rbind(dat, undiluted %>% filter(!samplecode %in% dat$samplecode)) %>%
  arrange(samplecode) %>%
  mutate(method = "IC",
         notes = "")
...
# To check for which samples needs to be remeasured due to high dl
d_IC3_wide %>% select(1:8) %>% filter(if_any(everything(), ~ str_detect(., "<"))) %>% view()
...
openxlsx::write.xlsx(d_IC3, paste0(output, "Clean_data/Third_fieldwork/IC_Oct-Jan_2024.xlsx"))
openxlsx::write.xlsx(d_IC3_wide, paste0(output, "Clean_data/Third_fieldwork/IC_Oct-Jan_2024_wide.xlsx"))
