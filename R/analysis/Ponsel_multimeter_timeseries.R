#
# Input: Ponsel Multimeter raw dataset from ODEON VIEWER
#         
# Output: Cleaned Multimeter file
# 
# Dependencies: none
#
#
...
filename2 <- "CUR3_16-01-2023.csv"

# alkalinity data file
dat <- read.csv(paste0(input, "Ponsel_multimeter/", filename),
                skip = 3)
dat2 <- read.csv(paste0(input, "Ponsel_multimeter/", filename2),
                 skip = 3)
...
d <- dat %>%
  rename("record" = "X.",
         "datetime" = "Date",
         "conductivity (�S/cm)" = "Conductivit�..�S.cm.",
         "salinity (ppt)" = "Salinit�..ppt.",
         "O2 (%)" = "O2...Sat.",
         "O2 (mg/L)" = "O2..mg.l.",
         "temp O2 (C)" = "Temp�rature...C.",
         "pH" = "PH..pH.",
         "redox (mV)" = "Redox..mV.") %>%
