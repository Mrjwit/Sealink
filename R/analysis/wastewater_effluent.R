#
# Input: Wastewater effluent data
#         
...
input <- "C:/Users/mikewit/Documents/SEALINK/Documents/Wastewater/" 

# read data pumping tests
dat <- read.xlsx(paste0(input, "Effluentverbruik Oost en West_2017_2019.xlsx")) %>%
  mutate(Usage = as.numeric(Usage)) %>%
  mutate(Period = ifelse(Period == "Sept-Okt", "Sep-Okt", Period)) %>%
  mutate(Period = factor(Period, levels = c("Jan-Feb",
                                               "Feb-Mrt",
                                               "Mrt-Apr",
                                               "Apr-Mei",
                                               "Mei-Jun",
...
ggplot(dat %>% filter(User == "Total"), 
  ...
  labs(title = "WWTP effluent usage, 2017",
       x = bquote(~m^3), y = "") + 
  theme_bw() +
...
ggplot(dat %>% filter(User == "Total"),
       aes(x = WWTP, y = Usage, fill = Period)) +
  geom_col() +
  labs(title = "WWTP Effluent usage, 2017",
       y = bquote(~m^3)) +
  theme_bw()
