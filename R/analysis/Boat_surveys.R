...
# 18-12-2023
p1 <- ggplot(df %>% filter(date == "2023-12-18"), aes(x = datetime2, y = `conductivity (ÂµS/cm)`)) +
  geom_line() +
  geom_point() +
  labs(x = "") +
  theme_bw()
...
# 22-12-2023
p1 <- ggplot(df %>% filter(date == "2023-12-22",
                           `conductivity (ÂµS/cm)` > 10), aes(x = datetime2, y = `conductivity (ÂµS/cm)`)) +
  geom_line() +
  geom_point() +
  labs(x = "") +
  theme_bw()
...
# EC
p1 <- ggplot() +
  geom_sf(data = pts %>% filter(!is.na(`conductivity (ÂµS/cm)`),
                                       date == "2023-12-18"), 
          aes(fill = `conductivity (ÂµS/cm)`, shape = date), 
          colour = "black",
          pch = 21,
          size = 2) +
  scale_fill_viridis() +
...
d_long <- d %>%
  pivot_longer(., cols = `conductivity (ÂµS/cm)`:`redox (mV)`,
               names_to = "param",
               values_to = "value") %>%
  # set param names as factors for plotting order
  mutate(param = factor(param, levels = c("conductivity (ÂµS/cm)",
                                          "salinity (ppt)",
                                          "O2 (mg/L)",
                                          "O2 (%)",
                                          "pH",
...
# EC
ggplot(d_long %>% filter(param == "conductivity (ÂµS/cm)",
                         value > 20), 
...
# radon
p1 <- ggplot(d_long %>% filter(param == "conductivity (ÂµS/cm)"), 
...
## graphs
# EC
ggplot(d, aes(x = datetime2, y = `conductivity (ÂµS/cm)`)) +
  geom_point() +
  geom_line() +
...
for(i in unique(d$series)) {
  ...
  ylim.prim <- c(min(d1$`conductivity (ÂµS/cm)`), 
                 max(d1$`conductivity (ÂµS/cm)`))   
  ...
  plt <- ggplot(d1, aes(datetime2, `conductivity (ÂµS/cm)`)) +
    #geom_point() +
...
write.csv(d %>% filter(series == 6) %>% 
            select(record, datetime, date, time, 'conductivity (ÂµS/cm)'),
